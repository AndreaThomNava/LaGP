...
from lagp.utils.generate_data import simulate_latentGP, simulate_response, generate_input_grid 
...
def gen_data(name_config_file: str):
    ...
    for sample_size in sim_config['sample_sizes']:
        for dim in sim_config['dimensions']:
            ...
            X = generate_input_grid(d = dim, sample_size=sample_size, lims = sim_config["grid_lims"])  # Grid of input points
            ...
            for likelihood in sim_config['likelihoods']:
                ...
                likelihood_dir = os.path.join(output_dir, f"{sample_size}_{dim}", likelihood)
                ...
                for replicate in range(sim_config['replicates']):
                    ...
                    # Save the data
                    output_file = os.path.join(likelihood_dir, f"data_replicate_{replicate + 1}.npz")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate simulated data based on configuration.")
