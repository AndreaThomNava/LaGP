...
def main():
    ...
    parser.add_argument("setting", nargs="?", default="simulation")
    parser.add_argument("version", nargs="?", default="001")
    parser.add_argument(
        "pickle_file_1",
        nargs="?",
        default="results_python.pkl",
        type=str,
    ...
    RESULTS_PATH = os.path.join("results", setting, version)
    # Load the pickle files
    with open(
        os.path.join(RESULTS_PATH, f"{setting}_{args.pickle_file_1}"), "rb"
    ) as f1:
        res_python = pickle.load(f1)

    with open(
        os.path.join(RESULTS_PATH, f"{setting}_{args.pickle_file_2}"), "rb"
    ) as f2:
    ...
    OUTPUT_PATH = os.path.join(RESULTS_PATH, f"{setting}_{args.output_file}")
